...
- **Region labels**: If region recognition is enabled (`region` column + `plate_regions`), validates missing/unknown regions.
- **Region config mismatch**: Warns if CSV has `region` but `plate_regions` is not configured.
